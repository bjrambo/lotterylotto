...
class lotterylottoAdminView extends lotterylotto
{
	...
	function dispLotterylottoAdminStats()
	{
		...
		foreach ($output->data as $key => $val)
		{
			...
			if ($val->result == 1)
			{
				$stats->first_cnt = $stats->first_cnt + 1;
			}
			if ($val->result == 2)
			{
				$stats->second_cnt = $stats->second_cnt + 1;
			}
			if ($val->result == 3)
			{
				$stats->third_cnt = $stats->third_cnt + 1;
			}
			if ($val->result == 4)
			{
				$stats->fourth_cnt = $stats->fourth_cnt + 1;
			}
			if ($val->result == 5)
			{
				$stats->fifth_cnt = $stats->fifth_cnt + 1;
			}
			if ($val->result == 0)
			{
				$stats->lose_cnt = $stats->lose_cnt + 1;
			}
