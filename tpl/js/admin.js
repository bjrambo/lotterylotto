...
function jsDeleteLotteryLog() {
    ...
    exec_xml('lotterylotto','procLotterylottoAdminLogDelete', params, completeDeletelog); //모듈이름//액션이름//보내줄값//콜백함수//콜백함수에서 받을변수(미입력시 message 기본내장)
}

// 전체로그삭제
function jsDeleteLotteryLogAll() {
    if(!confirm('전체로그를 삭제합니다.\n삭제후 데이터 복구는 불가능합니다.')) return;
    exec_xml('lotterylotto','procLotterylottoAdminLogDeleteAll', {}, completeDeletelog); //모듈이름//액션이름//보내줄값//콜백함수//콜백함수에서 받을변수(미입력시 message 기본내장)
}
