	...
	***********************************/
    ...
    class lotterylottoController extends lotterylotto {
		...
		function procLotterylottoBuyLottery() {
			...
			if(!$logged_info) return $this->setMessage('로그인후 이용가능합니다');
			...
			//일일구매횟수 초과시 리턴
			if($module_config->buy_limit_use == 'yes' && $today_count > $module_config->buy_limit_count) return $this->setMessage('일일 구매 횟수를 초과했습니다.');
			
			//머니확인
			$user_money = $oLotterylottoModel->checkUserMoney($module_config->money_type,$logged_info);
			
			//머니부족시 리턴
			if($module_config->buy_money > $user_money) return $this->setMessage('구매 비용이 부족합니다');
			...
			//로그등록
			executeQuery('lotterylotto.insert_lottery_log',$args);
			
			//당첨메세지설정
			if($result != 0) {
				$result_rank = $result.'등';
				$msg = str_replace(array('[rank]','[point]','[price]','[money]','[enter]'),array($result_rank,$prize_result->get_money,$module_config->buy_money,$module_config->money_lang,'<br>'),$module_config->win_msg);
			} else {
				$result_rank = '꽝';
				$msg = str_replace(array('[rank]','[point]','[price]','[money]','[enter]'),array($result_rank,$prize_result->get_money,$module_config->buy_money,$module_config->money_lang,'<br>'),$module_config->lose_msg);
			}
		...
		function procLotterylottoUpdateMsg(){
			...
			//본인이 아닐시 리턴
			$logged_info = Context::get('logged_info');
			if($logged_info->member_srl != $obj->member_srl) return;
			...
			if($obj->prize_msg && $obj->prize_msg != 'null'){
				...
				$obj->prize_msg = str_replace(array('[rank]','[point]','[price]','[money]','[enter]'),array($obj->result,$obj->get_money,$module_config->buy_money,$module_config->money_lang,'<br>'),$module_config->prize_msg);
			}
			
			//당첨소감입력
			executeQuery('lotterylotto.update_prizeMsg',$obj);
		}
