...
class lotterylottoModel extends lotterylotto
{
	...
	{
		if (!$money_type || !$change_type || !$money || !$logged_info)
		{
			return;
		}
		if ($money_type == 'point')
	...
	function getPrizeMoney($result, $total_money, $logged_info, $module_config)
	{
		...
		if (!$total_money)
		{
			$total_money = $module_config->emergency_money;
		}
		//설정한 포인트보다 작을시 비상누적금사용 (비상누적금에 기존적립된 누적금을 합침)
		if ($module_config->emergency_money_min_point > $total_money)
		{
			$total_money = $module_config->emergency_money + $total_money;
		}
		//1등당첨금 수령
		if ($result == '1')
		{
		...
		elseif ($result == '2')
		{
			if ($module_config->money_type2 == 'fix')
			{ //고정 당첨금
		...
		elseif ($result == '3')
		{
			if ($module_config->money_type3 == 'fix')
			{ //고정 당첨금
		...
		elseif ($result == '4')
		{
			if ($module_config->money_type4 == 'fix')
			{ //고정 당첨금
		...
		elseif ($result == '5')
		{
			if ($module_config->money_type5 == 'fix')
			{ //고정 당첨금
	...
	function makeLotteryResult($module_config)
	{
		...
		//꽝확률 오류 발생시 리턴
		if ($lose_per < 0 && $lose_per > 10000)
		{
			return $this->setMessage('꽝 확률 오류발생');
		}
